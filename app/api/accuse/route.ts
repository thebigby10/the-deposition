...
import { DIFFICULTIES, DifficultySchema, pickModel } from '@/lib/game';
import { z } from 'zod';

export const maxDuration = 30;

export async function POST(req: Request) {
  const { secret = '', accusation = '', apiKey, difficulty } = await req.json().catch(() => ({}));
  const diff = DIFFICULTIES[DifficultySchema.parse(difficulty)];
...
${diff.accusePrompt}
Fail it if the accusation names the wrong act, wrong motive, or is too
vague to count as an accusation at all.
